...
const {  getTBprogramasAll, getTBprogramasById } = require('./tbprogramas');
...
const popularSubjectProgram = async(  ) => {
 ...
 const popularAggrementProgram = async(  ) => { 
    
    const programas = await getTBprogramasAll();
    const programs = JSON.parse(programas);
    programs.map(async(program)=>{
        console.log('Programa',program.nombre);
        await setAggrementProgram(program.id);
    });

};

const setAggrementProgram = async(id)=>{
    try {
    
            const dataProgramas = await getTBprogramasById(id);
            const programas = JSON.parse(dataProgramas);
          
            programas.map( async(programa,i) => {
                    // const aggrements = await getAggrementByIdPlatform(programa.idconvenio);
                    // const aggrement = await JSON.parse(aggrements);

                    const aggrement = await Aggrement.findOne({idPlatform: programa.idconvenio})

                    console.log(aggrement);

                    if (aggrement){
                        const program = await Program.findOne({idPlatform: id});

                        program.aggrement = aggrement._id;
                        
                        await program.save();
                        console.log( programa.sigla,aggrement.shortName);
                    }else{
                        const program = await Program.findOne({idPlatform: id});

                        program.aggrement = null;
                        
                        await program.save();
                        console.log( programa.sigla,null);
                    }
